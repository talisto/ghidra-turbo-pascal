#Apply IDA FLIRT signatures for Ghidra (headless/batch mode)
# original code from
...
#@author NWMonster (additional fixes)
#@category FunctionID
#@menupath Tools.Function ID.ApplySigHeadless

# Usage (headless):
#   pyghidraRun -H <proj-dir> <proj-name> -process <EXE> \
#     -postScript ApplySigHeadless.py path/to/file.sig \
#     -scriptPath scripts
#
# Pass the .sig file path as the first script argument.
# IMPORTANT: Requires pyghidraRun -H (not analyzeHeadless) — Python scripts
# need PyGhidra's CPython bridge.
...
try:
	...
	pass
try:
	from itertools import izip, islice  # Python 2 / Jython
except ImportError:
	izip = zip                           # Python 3
	from itertools import islice
from io import BytesIO  # for compressed binary data
import zlib
import struct
...
def parse_flirt_file(f):
	...
	if header.features & FlirtFeatureFlag.FEATURE_COMPRESSED:
		# IDA FLIRT v5 uses raw deflate (no zlib header) — use wbits=-15
		f = BytesIO(zlib.decompress(f.read(), -15))
...
def ask_sig():
	"""In headless mode, reads .sig file path from getScriptArgs()[0]."""
	args = getScriptArgs()
	if args and len(args) > 0:
		...
		print("ERROR: No .sig file path provided.")
		print("Usage: -postScript ApplySigHeadless.py path/to/file.sig")
		return None
...
def get_function_end(funk):
	...
	max = 0
	for BB in BBs:
		bb_max = _addr_to_int(BB.getMaxAddress().toString())
		if bb_max > max:
			max = bb_max
	return max
...
def _addr_to_int(ghidra_addr_str):
	"""Parse Ghidra address string to int; handles 'seg:off' (DOS) and flat hex."""
	s = str(ghidra_addr_str)
	if ':' in s:
		seg, off = s.split(':')
...
def _int_to_addr(n):
	"""Reconstruct a Ghidra Address from int packed by _addr_to_int."""
	seg = (n >> 16) & 0xFFFF
	off = n & 0xFFFF
	if seg:
		return parseAddress('%04x:%04x' % (seg, off))
	return toAddr(n)
...
def funk_rename(addr, funk):
	...
	name = funk.name
	if name != '?':
		# Compute the actual address: module base + function offset within module
		target = addr
		if funk.offset != 0:
			# For segmented addresses, add offset to the offset portion
			seg = (addr >> 16) & 0xFFFF
			off = addr & 0xFFFF
			target = (seg << 16) | ((off + funk.offset) & 0xFFFF)
		func_obj = getFunctionAt(_int_to_addr(target))
		if func_obj is not None:
			try:
				func_obj.setName(name, SourceType.USER_DEFINED)
				rename_cnt += 1
			except Exception:
				pass  # name collision or other issue
	return

def apply_sig(flirt):
	funk = getFirstFunction()
	while funk is not None:
		funk_start = _addr_to_int(funk.entryPoint.toString())
		funk_end   = get_function_end(funk)
		funk_buf   = getBytes(_int_to_addr(funk_start), funk_end - funk_start + 0x100)
		match_function(flirt, funk_buf, funk_start, funk_rename)
		funk = getFunctionAfter(funk)

f = ask_sig()
if f is None:
	print("Aborting: no sig file.")
	raise SystemExit(1)
...
print('Count:', flirt.header.n_functions or flirt.header.old_n_functions)
print('Apply Signatures.....')
apply_sig(flirt)
print('[ %d renamed / %s total in sig ]' % (rename_cnt, flirt.header.n_functions or flirt.header.old_n_functions or '?'))
print('Done!')
